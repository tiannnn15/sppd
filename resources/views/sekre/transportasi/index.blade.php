    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Data Transport Taksi</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
      ...
      <div class="row">
        ...
        <div class="col-lg-12">
          <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                  <h5 class="card-title">Data Biaya Transport</h5>
                  <a href="{{ route('transportasi.create') }}" class="btn btn-md btn-primary mb-3">TAMBAH DATA</a>
                  ...
                  <table class="table datatable">
                    ...
                    <tbody>
                        <?php $no = 1 ?>
                      @forelse ($transportasi as $transportasi)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $transportasi->provinsi->nama_provinsi }}</td>
                            <td>{{ $transportasi->satuan }}</td>
                            <td>{{ $transportasi->besaran }}</td>
                            <td class="text-center">
                                <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('transportasi.destroy', $transportasi->id) }}" method="POST">
                                    <a href="{{ route('transportasi.edit', $transportasi->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                    @csrf
                                    @method('DELETE')
                      ...
                      @empty
                        <div class="alert alert-danger">
                            Data Transportasi belum Tersedia.
                        </div>
                      @endforelse
                    </tbody>
                  </table>
