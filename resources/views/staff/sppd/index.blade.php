          ...
          <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                  ...
                  <a href="{{ route('sppd.create') }}" class="btn btn-md btn-primary mb-3">TAMBAH DATA</a>
                  ...
                  <table class="table datatable">
                    ...
                    <tbody>
                        <?php $no = 1 ?>
                      @forelse ($sppd as $sppd)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $sppd->nomer_sppd }}</td>
                            <td>{{ $sppd->pelaksana->pegawai->nama_pegawai }}</td>
                            <td>{{ $sppd->maksud_perjalanan }}</td>
                            <td>{{ date("d-m-Y", strtotime($sppd->tgl_sppd)) }}</td>
                            ...
                            <td class="text-center">
                              <a href="{{ route('sppd.show', $sppd->id) }}" class="btn btn-sm btn-primary">SHOW</a>
                              <a href="{{ route('sppd.edit', $sppd->id) }}" class="btn btn-sm btn-warning">EDIT</a>
                              <a href="{{ route('sppd.pdf', $sppd->id) }}" class="btn btn-sm btn-success">DOWNLOAD</a>
                              <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('sppd.destroy', $sppd->id) }}" method="POST" class="d-inline">
                              @csrf
                                    @method('DELETE')
